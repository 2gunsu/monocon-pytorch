...
If the directory ```_C.OUTPUT_DIR``` in ```monocon_configs.py``` already exists and incomplete checkpoints are found, it will automatically restart up to the target epoch by loading the latest checkpoint.  
```bash
python train.py
```
